...
public class AdministratorManager {
     ...
     */
	public static void putAdministrator(Administrator administrator) 
			throws ObjectExistsInDatastoreException {
		PersistenceManager pm = PMF.get().getPersistenceManager();
    ...
    */
	public static void deleteAdministrator(Email email) {	
		...
		try {
			...
			Administrator administrator = 
					pm.getObjectById(Administrator.class, key);
			tx.begin();
			pm.deletePersistent(administrator);
			tx.commit();
			log.info("Administrator \"" + email.getEmail() + 
    ...
    * @param key
    * 			: the key of the administrator whose password will be changed
    ...
    */
	public static void updateAdministratorPassword(Key key, String currentPassword,
		...
		try {
			...
			tx.begin();
			if (administrator.getUser().validateUser(
					administrator.getUser().getUserEmail(), currentPassword)) {
				administrator.getUser().setUserPassword(newPassword);
				tx.commit();
				log.info("Administrator \"" + administrator.getAdministratorName() + 
						"\"'s password updated in datastore.");
			}
			else {
				tx.rollback();
				throw new UserValidationException(administrator.getUser(), 
	...
	* @throws MissingRequiredFieldsException
    */
	public static void updateAdministratorPassword(Key key, String newPassword) 
			throws MissingRequiredFieldsException {	
    ...
    * @param key
    * 			: the key of the administrator whose attributes will be updated
    ...
    * 			: the new comments to give to the administrator
	 * @throws MissingRequiredFieldsException 
    */
	public static void updateAdministratorAttributes(Key key, String administratorName,
