...
function encodeData()
{
	...
	if (document.getElementsByName("type")[0].value.trim() === "per") {
		content.value =
	...
	else if (document.getElementsByName("type")[0].value.trim() === "aper") {
		content.value =
  			document.getElementsByName("device_id")[0].value + "," +
  			document.getElementsByName("type")[0].value + "," +
  			document.getElementsByName("door_number")[0].value + "," +
  			document.getElementsByName("door_status")[0].value;
	}
...
<form method="post" id="form1" name="form1" 
    ...
    <fieldset>
    <legend><%= printer.print("Sensor Data Upload Test") %></legend>
	...
	<div>
		...
		<input type="text" name="type" class="input_extra_large" value="per" /><br />
		<div id="type"></div>
	</div>
	...
	<div>
        <label for="door_number"><span><%= printer.print("Door Number") %></span></label>
		<input type="text" name="door_number" class="input_extra_large" value="1" /><br />
		<div id="door_number"></div>
	</div>
	
	<div>
        <label for="door_status"><span><%= printer.print("Door Status") %></span></label>
		<input type="text" name="door_status" class="input_extra_large" value="Open" /><br />
		<div id="door_status"></div>
	</div>
	
	<div>
        <label for="initialUpload"><span><%= printer.print("Initial upload") %></span></label>
		<input name="initialUpload" type="checkbox" id="initialUpload"/><br/>
		<div id="door_status"></div>
	</div>
